...
int debug_flag = 0;                /* 调试模式 */
int logfd = -1;                    /* 日志文件描述符 */
char *logfilename = NULL;          /* 日志文件名 */
 ...
 */
static int httpsqs_maxqueue(const char *httpsqs_input_name,
                            int httpsqs_input_num) {
     ...
     */
    if (queue_maxnum_int >= queue_put_value &&
        queue_maxnum_int >= queue_get_value &&
...
static int httpsqs_now_putpos(const char *httpsqs_input_name) {
    ...
    if (queue_put_value ==
        queue_get_value) { /* 如果队列写入ID+1之后追上队列读取ID，则说明队列已满，返回0，拒绝继续写入
                            */
        queue_put_value = 0;
    } else if (
        queue_get_value <= 1 &&
        queue_put_value >
            maxqueue_num) { /* 如果队列写入ID大于最大队列数量，并且从未进行过出队列操作（=0）或进行过1次出队列操作（=1），返回0，拒绝继续写入
                             */
        queue_put_value = 0;
    } else if (
        queue_put_value >
        maxqueue_num) { /* 如果队列写入ID大于最大队列数量，则重置队列写入位置点的值为1
                         */
...
void httpsqs_handler(struct evhttp_request *req, void *arg) {
    ...
    /* 接收GET表单参数 */
    const char *httpsqs_input_auth =
        evhttp_find_header(&httpsqs_http_query, "auth"); /* 队列名称 */
    const char *httpsqs_input_name =
        evhttp_find_header(&httpsqs_http_query, "name"); /* 队列名称 */
    const char *httpsqs_input_charset =
        evhttp_find_header(&httpsqs_http_query, "charset"); /* 操作类别 */
    const char *httpsqs_input_opt =
        evhttp_find_header(&httpsqs_http_query, "opt"); /* 操作类别 */
    const char *httpsqs_input_data =
        evhttp_find_header(&httpsqs_http_query, "data"); /* 操作类别 */
    const char *httpsqs_input_pos_tmp =
        evhttp_find_header(&httpsqs_http_query, "pos"); /* 队列位置点 字符型 */
    const char *httpsqs_input_num_tmp =
        evhttp_find_header(&httpsqs_http_query, "num"); /* 队列总长度 字符型 */
    ...
    if (httpsqs_settings_auth != NULL) {
        /* 如果命令行启动参数设置了验证密码 */
        if (httpsqs_input_auth != NULL &&
            strcmp(httpsqs_settings_auth, httpsqs_input_auth) == 0) {
            is_auth_pass = true;
        } else {
            is_auth_pass = false;
        }
    ...
    if (is_auth_pass == false) {
        ...
        if (httpsqs_input_pos_tmp != NULL) {
            httpsqs_input_pos =
                atoi(httpsqs_input_pos_tmp); /* 队列位置点 数值型 */
        }
        if (httpsqs_input_num_tmp != NULL) {
            httpsqs_input_num =
                atoi(httpsqs_input_num_tmp); /* 队列总长度 数值型 */
        }

        /*参数是否存在判断 */
        if (httpsqs_input_name != NULL && httpsqs_input_opt != NULL &&
            strlen(httpsqs_input_name) <= 256) {
            /* 入队列 */
            if (strcmp(httpsqs_input_opt, "put") == 0) {
                ...
                if (buffer_data_len > 0) {
                    int queue_put_value =
                        httpsqs_now_putpos((char *)httpsqs_input_name);
                    if (queue_put_value > 0) {
                        char queue_name[300] = {
                            0}; /* 队列名称的总长度，用户输入的队列长度少于256字节
                                 */
                        sprintf(queue_name, "%s:%d", httpsqs_input_name,
                                queue_put_value);
                        char *httpsqs_input_postbuffer;
                        char *buffer_data =
                            (char *)tccalloc(1, buffer_data_len + 1);
                    ...
                    if (queue_put_value > 0) {
                        ...
                        char *buffer_data =
                            (char *)tccalloc(1, buffer_data_len + 1);
                        memcpy(buffer_data, httpsqs_input_data,
                               buffer_data_len);
                        httpsqs_input_postbuffer = urldecode(buffer_data);
                        tcbdbput2(httpsqs_db_tcbdb, queue_name,
                                  httpsqs_input_postbuffer);
            ...
            /* 出队列 */
                ...
                int queue_get_value = 0;
                queue_get_value =
                    httpsqs_now_getpos((char *)httpsqs_input_name);
                if (queue_get_value == 0) {
                    ...
                    httpsqs_output_value =
                        tcbdbget2(httpsqs_db_tcbdb, queue_name);
            ...
            /* 查看队列状态（普通浏览方式） */
                ...
                int maxqueue = httpsqs_read_maxqueue(
                    (char *)httpsqs_input_name); /* 最大队列数量 */
                int putpos = httpsqs_read_putpos(
                    (char *)httpsqs_input_name); /* 入队列写入位置 */
                int getpos = httpsqs_read_getpos(
                    (char *)httpsqs_input_name); /* 出队列读取位置 */
                int ungetnum;
                ...
                if (putpos >= getpos) {
                    ...
                    ungetnum =
                        abs(maxqueue - getpos + putpos); /* 尚未出队列条数 */
                    put_times = "2nd lap";
                    get_times = "1st lap";
                }
            ...
            /* 查看队列状态（JSON方式，方便客服端程序处理） */
                ...
                evbuffer_add_printf(
                    buf,
                    "{\"name\":\"%s\",\"maxqueue\":%d,\"putpos\":%d,\"putlap\":"
                    "%s,"
                    "\"getpos\":%d,\"getlap\":%s,\"unread\":%d}\n",
            ...
            /* 查看单条队列内容 */
                     ...
                     httpsqs_input_pos >= 1 &&
                     httpsqs_input_pos <= 1000000000) {
                char *httpsqs_output_value;
                httpsqs_output_value =
                    httpsqs_view((char *)httpsqs_input_name, httpsqs_input_pos);
                if (httpsqs_output_value) {
                    evbuffer_add_printf(buf, "%s", httpsqs_output_value);
                    free(httpsqs_output_value);
                }
            ...
            /* 设置最大的队列数量，最小值为10条，最大值为10亿条 */
            else if (strcmp(httpsqs_input_opt, "maxqueue") == 0 &&
                     httpsqs_input_num >= 10 &&
                     httpsqs_input_num <= 1000000000) {
                if (httpsqs_maxqueue((char *)httpsqs_input_name,
                                     httpsqs_input_num) != 0) {
                    /* 设置成功 */
             ...
             */
            else if (strcmp(httpsqs_input_opt, "synctime") == 0 &&
                     httpsqs_input_num >= 1 &&
                     httpsqs_input_num <= 1000000000) {
...
static void show_help(void) {
    char *b =
        ...
        "-s <second>   the interval to sync updated contents to the disk "
        "(default: 5)\n"
        "-c <num>      the maximum number of non-leaf nodes to be cached "
        "(default: 1024)\n"
        "-m <size>     database memory cache size in MB (default: 100)\n"
        "-i <file>     save PID in <file> (default: /tmp/httpsqs.pid)\n"
        "-a <auth>     the auth password to access httpsqs (example: "
        "mypass123)\n"
        "-d            run as a daemon\n"
        "-h            print this help and exit\n\n"
        "Use command \"killall httpsqs\", \"pkill httpsqs\" and \"kill `cat "
        "/tmp/httpsqs.pid`\" to stop httpsqs.\n"
        "Please note that don't use the command \"pkill -9 httpsqs\" and "
        "\"kill "
        "-9 PID of httpsqs\"!\n"
        "\n"
        "Please visit \"http://code.google.com/p/httpsqs\" for more help "
        "information.\n\n"
        "----------------------------------------------------------------------"
        "--"
        "--------------------------\n"
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    int httpsqs_settings_cacheleaf =
        2048; /* 缓存叶子节点数。叶子节点缓存数为非叶子节点数的两倍。单位：条 */
    ...
    char httpsqs_path_file[1024] = {
        0};  // httpsqs_path_file 为 httpsqs 程序的绝对路径
    ...
    for (httpsqs_prename_num = 1; httpsqs_prename_num < argc;
         httpsqs_prename_num++) {
        evbuffer_add_printf(httpsqs_prename_buf, " %s",
                            argv[httpsqs_prename_num]);
    }

    /* process arguments */
    while ((c = getopt(argc, argv, "l:p:x:t:s:c:m:i:a:d:g:h")) != -1) {
        switch (c) {
            case 'l':
            ...
            case 'p':
                ...
                break;
            case 'x':
                httpsqs_settings_datapath =
                    strdup(optarg); /* httpsqs数据库文件存放路径 */
                if (access(httpsqs_settings_datapath, W_OK) !=
                    0) { /* 如果目录不可写 */
                    if (access(httpsqs_settings_datapath, R_OK) ==
                        0) { /* 如果目录可读 */
                        chmod(httpsqs_settings_datapath,
                              S_IWOTH); /* 设置其他用户具可写入权限 */
                    } else { /* 如果不存在该目录，则创建 */
                    ...
                    if (access(httpsqs_settings_datapath, W_OK) !=
                        0) { /* 如果目录不可写 */
                        fprintf(stderr,
                                "httpsqs database directory not writable\n");
                    }
                }
                break;
            case 't':
            ...
            case 'c':
                ...
                break;
            case 'm':
                httpsqs_settings_mappedmemory =
                    atoi(optarg) * 1024 * 1024; /* 单位：M */
                break;
            case 'i':
            ...
            case 'g':
                ...
                logfd = open(logfilename, O_RDWR | O_CREAT | O_APPEND, 0644);
                log_message("log mod on\n");
                break;
            case 'h':
            default:
                show_help();
    ...
    char *httpsqs_settings_dataname =
        (char *)tccalloc(1, httpsqs_settings_dataname_len);
    sprintf(httpsqs_settings_dataname, "%s/httpsqs.db",
            httpsqs_settings_datapath);
    ...
    tcbdbsetcache(httpsqs_db_tcbdb, httpsqs_settings_cacheleaf,
                  httpsqs_settings_cachenonleaf);
    tcbdbsetxmsiz(httpsqs_db_tcbdb,
                  httpsqs_settings_mappedmemory); /* 内存缓存大小 */

    /* 判断表是否能打开 */
    if (!tcbdbopen(httpsqs_db_tcbdb, httpsqs_settings_dataname,
    ...
    prename_setproctitle("[httpsqs: master process] %s",
                         (char *)EVBUFFER_DATA(httpsqs_prename_buf));
    ...
    prename_setproctitle_init(argc, argv, envp);
    prename_setproctitle("[httpsqs: worker process] %s",
                         (char *)EVBUFFER_DATA(httpsqs_prename_buf));
