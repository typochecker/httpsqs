  ...
  if (is_auth_pass == false) {
    ...
    if (httpsqs_input_name != NULL && httpsqs_input_opt != NULL &&
      ...
      /* 查看队列状态（JSON方式，方便客服端程序处理） */
        ...
        evbuffer_add_printf(
            buf,
            "{\"name\":\"%s\",\"maxqueue\":%d,\"putpos\":%d,\"putlap\":%s,"
            "\"getpos\":%d,\"getlap\":%s,\"unread\":%d}\n",
            httpsqs_input_name, maxqueue, putpos, put_times, getpos, get_times,
            ungetnum);
      }
      /* 查看单条队列内容 */
      else if (strcmp(httpsqs_input_opt, "view") == 0 &&
               httpsqs_input_pos >= 1 && httpsqs_input_pos <= 1000000000) {
        char *httpsqs_output_value;
      ...
      /* 设置最大的队列数量，最小值为10条，最大值为10亿条 */
               ...
               httpsqs_input_num >= 10 && httpsqs_input_num <= 1000000000) {
        if (httpsqs_maxqueue((char *)httpsqs_input_name, httpsqs_input_num) !=
            0) {
          /* 设置成功 */
      ...
      /* 设置定时更新内存内容到磁盘的间隔时间，最小值为1秒，最大值为10亿秒 */
               ...
               httpsqs_input_num >= 1 && httpsqs_input_num <= 1000000000) {
...
static void show_help(void) {
  char *b =
      ...
      "Please note that don't use the command \"pkill -9 httpsqs\" and \"kill "
      "-9 PID of httpsqs\"!\n"
      "\n"
